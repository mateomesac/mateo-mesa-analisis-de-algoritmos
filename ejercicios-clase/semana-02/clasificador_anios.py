...
def leer_anios() -> list[int]:
    ...
    datos_entrada = input("Ingrese los años separados por comas: ") 

    try: 
        anios_lista = [int(anio.strip()) for anio in datos_entrada.split(",")] 
        return anios_lista 
    except ValueError: 
...
def main() -> None:
    ...
    anios_lista = leer_anios()

    anios_bisiestos = [
        anio for anio in anios_lista if es_bisiesto(anio)
    ]
    print(f"Años ingrasados: {anios_lista}") 
    print(f"Años bisiestos: {anios_bisiestos}") 
    print(f"Cantidad de años bisiestos: {len(anios_bisiestos)} de {len(anios_lista)}")
